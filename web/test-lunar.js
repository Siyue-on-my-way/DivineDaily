...
// 测试1: 检查1993年的闰月
const leapMonth1993 = LunarCalendar.getLeapMonth(1993);
console.log(`1993年闰月: ${leapMonth1993}月`);
console.log(`预期: 3月`);
console.log(`结果: ${leapMonth1993 === 3 ? '✅ 通过' : '❌ 失败'}\n`);

// 测试2: 检查闰三月的天数
const leapDays1993 = LunarCalendar.getLeapDays(1993);
...
try {
  const result = LunarCalendar.lunarToSolar(1993, 3, 15, true);
...
try {
  const result = LunarCalendar.solarToLunar(1993, 5, 6);
...
testDates.forEach(({ date, expected, isLeap }) => {
  ...
  const result = LunarCalendar.solarToLunar(y, m, d);
